...
/**
* function to be executed when the page in loaded.
*/
function init() {

    // create a map and set the loading position
    ...
    /**
    * function which does the distance and all the necessary
    * things on the map.
    */
    function onPolygonLoad(json, myGeocode, myAddress) {


        mkad_km = json.coordinates;
        moscowPolygon = new ymaps.Polygon(mkad_km);
        // we set the mkad visible on the map, we set the appropriate option.
        moscowPolygon.options.set('visible', true);
        myMap.geoObjects.add(moscowPolygon);

        // allow our polygon to call the geometry functions
        moscowPolygon.geometry.setMap(myMap);


        /**
         * check if the typed address is located inside our mkad polygon,
         * if yes, do the process if not, display a message.
         */
        if (!moscowPolygon.geometry.contains(myGeocode)) {

            /**
             * loop through the MKAD polygon coordinates and add each
             * location to the map (as hidden object)
             */
            var arPlacemarks = new Array();
            for (i = 0; i < json.coordinates[0].length; i++)
                arPlacemarks[i] = new ymaps.Placemark(json.coordinates[0][i]);
            var arPlacemarksRez = ymaps.geoQuery(arPlacemarks).addToMap(myMap).setOptions('visible', false);


            // find the closest marker in MKAD polygon to the requested address(input)
            var closestObject = arPlacemarksRez.getClosestTo(myGeocode);


            ymaps.route([closestObject.geometry.getCoordinates(), myGeocode]).then(
                function(res) {
                    ...
                    // get the distance in kilometers and display it in the page
                    var distance_km = (res.getLength() / 1000).toFixed(2);
                    document.getElementById('km-value').innerHTML = distance_km

                    /**
                     * Add to the map the following:
                     * - sections that define the route;
                     * - the input address and the closest point in the MKAD;
                     * - intersections if the closest MKAD point needs to go through
                     * a longer road.
                     */
                    myPlacemark_input = new ymaps.Placemark(myGeocode, {
                            iconContent: 'Address',
                        }, {
                            preset: "islands#redStretchyIcon"
                        }),

                    myPlacemark_closest = new ymaps.Placemark(closestObject.geometry.getCoordinates(), {
                            iconContent: distance_km + ' Km',
                        }, {
                            preset: 'islands#lightBlueStretchyIcon',
                        });

                    myMap.geoObjects
                        .add(myPlacemark_input)
                        .add(myPlacemark_closest);
                    ...
                    /**
                     *  communicate with the backend to write
                     *  the input and results into the log file
                     *  using ajax
                     */
            ...
            // display a simple message if the entry location is inside MKAD
            distance_km = '0 (The address is located inside the MKAD)'
            document.getElementById('km-value').innerHTML = distance_km
        }
    ...
    /**
     * check whether there is a valid input that was correctly
     * processed in the python backend side.
     */
    if (typeof myGeocode !== 'undefined') {
        ...
        onPolygonLoad(mkad_km, [Number(myGeocode[1]), Number(myGeocode[0])], myAddress)
    }
}